...
enum class StompCommand {
    CONNECT, CONNECTED, SEND, MESSAGE, SUBSCRIBE, UNSUBSCRIBE, UNKNOWN;

    companion object {
        @JvmStatic
        fun fromString(command: String?): StompCommand {
            return try {
                if (command == null) UNKNOWN else valueOf(command.uppercase())
